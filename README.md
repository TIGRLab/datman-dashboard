...
2. [TIGRLab Dashboard](#tigrlab-dashboard)
3. [TIGRLab Ansible Configuration](#tigrlab-ansible-configuration)  
...
## Dashboard Structure
`dashboard/models.py` defines the _Object Relational Mapping (ORM)_ used by
...
HTML templates are in `dashboard/templates/`. Templates with `_snip.html` are
...
This section under construction. - Dawn, May 23 2018
...
# More notes here about how to run the devel environment.. i.e. what script to
use and to access it by the right oauth name etc.
...
of the real database (ask Dawn for this). Once you have the backup
...
Currently this page has advice on the config files and setting one up: https://github.com/TIGRLab/admin/wiki/data-organization-and-config-files

To do: Add wiki page here https://github.com/TIGRLab/datman/wiki/Datman-Config
...
from the template provided in `dashboard/dashboard.module.template`. Just
