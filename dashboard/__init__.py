...
if app.debug:
    ## This makes debug mode very, very noisy but can be helpful to uncomment if
    ## you're having database query issues
    ...
    if app.env == 'development':
        ...
        try:
            ...
            app.logger.warning("flask-debugtoolbar not installed. "
                    "Install it for extra development server goodness :)")
        else:
        ...
        if not os.path.exists(log_dir):
            app.logger.warning("{} does not exist. Make it to receive log files "
                    "while in development mode :)".format(log_dir))
        else:
            file_handler = RotatingFileHandler(os.path.join(log_dir,
                    'dashboard.log'), 'a', 1 * 1024 * 1024, 10)
    ...
    mail_handler = SMTPHandler((LOG_MAIL_SERVER, LOG_MAIL_PORT), SENDER, ADMINS,
            'Dashboard failure', credentials)
